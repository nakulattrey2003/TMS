...
// List of statuses
const statuses = ['In Transit', 'Delivered', 'Pending', 'Out for Delivery', 'Delayed'];
...
export async function loadShipmentsFromAPI() {
  try {
    ...
    shipments = expandedProducts.map((product, index) => {
      ...
      const shipDate = new Date(Date.now() - getRandomNumber(1, 10) * 24 * 60 * 60 * 1000);
      const deliveryDate = new Date(shipDate.getTime() + getRandomNumber(3, 7) * 24 * 60 * 60 * 1000);
      ...
      return {
        ...
        actualDelivery: status === 'Delivered' ? deliveryDate.toISOString() : null,
